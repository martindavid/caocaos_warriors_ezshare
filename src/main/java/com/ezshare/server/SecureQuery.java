...
public class SecureQuery {
	...
	public SecureQuery(Resource resource, boolean relay) {
	...
	public ArrayList<Resource> getResourceList() {
		...
		for (Resource res : Storage.resourceList) {
			...
			if (isMatch(newRes, this.resource)) {
				...
				Logger.debug(String.format(
						"QUERY: Resource Match - Channel: %s, Owner: %s, Uri: %s, Name: %s, Description: %s",
		...
		if (relay) { // Fetch resource from other server
			try {
				...
				for (ServerList server : Storage.secure_serverList) {
